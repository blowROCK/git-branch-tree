...
브랜치 이름의 `/` 를 **임의 뎁스 폴더 트리**로 펼쳐 보여주고, **leaf 브랜치를 클릭하면 체크아웃**하는 경량 VS Code 익스텐션.

기존 브랜치 트리 익스텐션들이 1뎁스만 접는 한계를, `feature/alerts/news/STK-380` 같은 3~4뎁스 전략에서도 제대로 펼쳐 해결합니다.

## 스크린샷

| Local | Remote |
| --- | --- |
| <img src="screenshot/screenshot_2.png" width="420" alt="다단계 폴더 트리로 펼쳐진 로컬 브랜치" /> | <img src="screenshot/screenshot_1.png" width="420" alt="릴리스 브랜치 아래 커밋이 보이는 Remote 섹션" /> |
| 다단계 폴더 트리로 펼쳐진 로컬 브랜치, 현재 브랜치 아래 최근 커밋 표시. | 동일한 트리의 Remote 섹션, `release` 브랜치 아래 커밋 표시. |
...
- **커밋 히스토리**: 브랜치를 펼치면 최근 커밋을 보여줍니다(한 번에 5개). **더보기** 노드로 다음 페이지를 불러옵니다.
- **브랜치 삭제**:
...
- **전체 펼치기**: 툴바 `expand-all` 버튼으로 모든 폴더를 한 번에 엽니다.
...
VS Code 에서 이 폴더를 열고 **F5** → Extension Development Host 가 뜨고, 활동 막대에 Branch Tree 아이콘이 생깁니다.

## 패키징(.vsix 사이드로드)

```bash
npm run vsix        # release/git-branch-tree-<version>.vsix 생성
code --install-extension release/git-branch-tree-*.vsix
```

## 범위

체크아웃 · 삭제(로컬/원격) · 새 브랜치 생성 · fetch+prune · 커밋 히스토리. 이름변경/별도 검색창은 제외(검색은 트리뷰 내장 타입어헤드로 충분). 단일 repo 가정.
